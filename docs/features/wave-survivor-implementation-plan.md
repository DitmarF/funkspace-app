...
| **WS-2.7**  | Implement the fixed virtual joystick.              | Use one primary pointer, pointer capture, a dead zone, clamped knob displacement, and normalized output. | Pointer leaving the original region does not break control; cancellation stops movement. |
...
| **WS-5.6**  | Define the first upgrade set.             | Start with attack cooldown, projectile damage, movement speed, health/heal, projectile size, and optional extra projectile. | Upgrades modify existing systems; no new economy or inventory layer is created.                      |

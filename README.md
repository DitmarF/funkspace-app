...
- Theme validation: `pnpm test:theme-bootstrap` checks source, shipped code, generator/watch behavior and dedicated 100% startup/DOM-adapter coverage. After `pnpm build` and browser setup, `pnpm e2e:theme-bootstrap` runs the production route on port 3100 with timing traces and a negative control. The existing `pnpm e2e` still uses the development server on port 3000.
- File flow and evidence: [typed theme bootstrap maintenance](docs/tasks/maintenance-theme-bootstrap.md). TypeScript source becomes generated script text; the small server-rendered wrapper delivers it inline. ThemeService takes over after startup. [Sites findings and acceptance handoff](docs/tasks/maintenance-theme-bootstrap.md#sites-review-incorporation--2026-09-15) retain the first-paint limitation and deferred runtime persistence work.

...
| Mail provider, recipient, secure secrets/configuration; existing host is Vercel | Dimi; Codex proposes integration    | FS-5.2 live adapter verification and FS-5.6         | Mocked contract/UI tests                             |
...
**Current status:** Pending Dimi decisions/input record. Existing FS-G0/branch and particular hosting approvals do not answer all product/content/operating choices.
...
- **Recommended first step:** read this draft, FS-0.4 and the baseline/branch links; verify checkout `8a054ae755315b334f18ac4b53ac4cce541e3519` plus the supplied documentation patch before counterpart review. Do not assume uncommitted files are shared.
...
- **Dimi next action:** provide the missing source when available and the remaining preservation information; FS-0.5 owns unapproved language/routes/content/device/service choices. English, 48-pixel targets, DPR cap 2, numeric scene tuning, required contact fields and service semantics remain proposals/defaults where not explicitly approved.

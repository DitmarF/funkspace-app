...
- **Status:** Draft — single intended milestone scope/status home; final publication is BLOCKED on reconciliation of the unavailable detailed EPIC 0 source. This draft preserves all 44 tasks from the accessible minimum-usable source. It does not assert that the missing source was read.
...
- **Complete source read:** `FunkSpace_Minimum_Usable_Development_Plan.md`, prepared 2026-09-11, local `/Users/dimi/Downloads/FunkSpace_Minimum_Usable_Development_Plan.md`; SHA-256 `6475a13f4538c5d3a3dbfb88be3be971e0291ddca4db27a131185934bf4c8ae1`. Its scope, 44 task IDs, leads, counterpart/Dimi responsibilities, dependencies, work and completion criteria are retained below.
- **Missing source:** `FunkSpace_EPIC_0_Detailed_Plan.md` remains inaccessible in the inspected repository, project sources, Downloads and Documents. Dimi reported uploading it and will add the local source later. No detailed-plan content is inferred from task prompts. Dimi supplies it; Codex reconciles it before FS-0.4 publication/closure, reporting any conflict instead of silently rewriting scope.
...
This is the single authoritative FS-0.5 decision/input register, expanded from the existing table. The [task record](../tasks/fs-0.5-product-and-technical-decisions.md) owns evidence and technical tradeoffs, not a second status register. Updated against clean input `65eefa1db2656e78b169882414dfedb4715aa408` plus the FS-0.5 documentation diff. Approved means an actual Dimi answer or a linked recorded approval; provisional means a reversible proposal, not implementation authorization. Pending rows name the owner and the latest task that needs the answer. The original task contracts below remain unchanged.
...
| Missing detailed source and preservation information      | Pending evidence; FS-0.4 publication BLOCKED             | Detailed EPIC 0 source still unavailable; known clone/untracked work protected; undisclosed local assets/work unconfirmed                        | Dimi supplies; Codex reconciles                                                           | FS-0.1/0.4 evidence and current protected-work snapshot                                                                                                    | Source before FS-0.4 closure; preservation before modifying affected area               | Does not revoke recorded FS-G0; blocks unsupported publication/affected edits                  | Accessible inspection, this decision record and unrelated preparation |

**FS-G0 decision distinction:** no additional Dimi strategy/language/route/target/device selection is required to repeat the already recorded FS-G0 approval. Source reconciliation and preservation evidence remain outstanding at their own deadlines. Later policy, renderer, contact, provider and visual approvals above are not approved by that gate or by this register. Dimi can supply assets/copy incrementally; secrets and finished legal content are not requested in chat now.

## Risks

| Risk                                                            | Likelihood / impact  | Mitigation                                                                                       | Verification                                              |
| --------------------------------------------------------------- | -------------------- | ------------------------------------------------------------------------------------------------ | --------------------------------------------------------- |
| Missing detailed source or treating draft as approval           | Present / high       | Qualified draft; Codex reconciles supplied source and Sites review before publication            | FS-0.4 record                                             |
...
| ----- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FS-G0 | **Approved by Dimi** | Explicit “FS-G0 is approved” recorded in FS-0.3. Integration subsequently executed/validated. Missing source reconciliation and preservation disclosures remain visible; approval does not fabricate their completion. |
...
The source sequence anticipated all EPIC 0 planning before FS-G0. Actual Dimi approval preceded FS-0.4/0.5 completion. Preserve both the source prerequisites and the recorded approval; do not silently treat either missing task as done or reopen the human decision. Remaining material choices still constrain their consumer tasks.
...
**Current status:** Inventory/reuse review delivered; source reconciliation and Dimi preservation details remain pending. [Evidence](../tasks/fs-0.1-inventory-real-starting-point.md).
...
**Current status:** BLOCKED for final publication on missing detailed source; safe draft, guidance reconciliation and read-only counterpart findings incorporated. [Execution and counterpart handoff](../tasks/fs-0.4-authoritative-feature-plan.md).
...
**Current status:** Product answers and technical decision preparation delivered in the [canonical register](#pending-decisions-and-latest-needed-points) and [FS-0.5 record](../tasks/fs-0.5-product-and-technical-decisions.md). Later inputs/approvals have owners and deadlines; Dimi remains lead/approver. FS-0.4 source reconciliation remains pending; no future architecture or implementation approval is inferred.
...
- **Recommended first step:** read this draft, the decision register, FS-0.4/0.5 and the baseline/branch links; FS-0.5 inspected `65eefa1db2656e78b169882414dfedb4715aa408` plus its supplied documentation patch. Verify the actual checkout/diff before counterpart review; do not assume uncommitted files are shared. The earlier FS-0.4 input above is historical provenance.
- **Codex next action:** reconcile the complete detailed EPIC 0 source when supplied, preserve the 44-task graph, resolve read-only Sites findings as sole writer, rerun documentation checks and then remove the publication blocker only with evidence.
- **Sites review:** recover scope, owners, dependencies, inherited base and pending choices from these repository files; return actionable findings read-only. Review sufficiency is not Dimi's visual acceptance.
- **Dimi next action:** provide the missing source when available and remaining preservation information. Language, routes, public-release target and representative devices are answered; the [decision register](#pending-decisions-and-latest-needed-points) schedules assets/content, mobile browser and later technical approvals. No repeat FS-G0 vote or immediate provider/secret submission is needed. Control targets, numeric tuning, contact fields and service semantics remain provisional or pending at their named tasks.
